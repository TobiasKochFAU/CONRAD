 ...
 * @author Marco Boegel, Frank Schebesch
 *
 */
...
public class exercise6 {
	...
	public void setSinogramParams(Grid2D sino, double focalLength, double maxRot)
	{
		...
		this.maxBeta = Math.PI + 2 * detectorSpacing;//TODO
	...
	public static void main(String arg[])
	{
		...
		Grid2D sino = fbp.readImageFile(imageDataLoc + "Sinogram0.tif");
		sino.show("Sinogram");
		
		// transpose so that each row is a projection
		sino = sino.getGridOperator().transpose(sino); // warning "Error in transpose" can be ignored
		sino.show();
