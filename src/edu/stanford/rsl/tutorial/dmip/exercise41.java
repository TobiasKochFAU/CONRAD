...
public class exercise41 {
	...
	public SimpleVector commonRansac(SimpleMatrix points, int mn, double p_opt, double p_out){
		...
		it = (int) (Math.log(1 - p_opt) / Math.log(1-Math.pow(1-p_out,points.getRows())));
	...
	public double lineError(SimpleVector line_params, SimpleMatrix points){
		...
		SimpleVector direction = new SimpleVector(1, m);
		...
		for (int i = 0; i < points.getRows(); i++)
		{
			SimpleVector x1 = new SimpleVector(0,c);
			SimpleVector x0 = points.getRow(i);
			SimpleMatrix sm = new SimpleMatrix(2, 2);
			sm.setColValue(0, direction);
			x1.subtract(x0);
			sm.setColValue(1, x1);
			double d = Math.sqrt(Math.abs(sm.determinant() / direction.normL2()));
			if(d > thresh) {
				err += d;
			}
		}
