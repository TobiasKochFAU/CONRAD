...
/**
 * Exercise 8-2 of Diagnostic Medical Image Processing (DMIP)
...
public class exercise82 {
	...
	private double calculateMutualInformation(Grid2D ref, Grid2D mov){
		...
		return mutual_information * 1000 * -1;
	}
	...
	private SimpleMatrix calculateJointHistogram(Grid2D im1, Grid2D im2){
		...
		for (int i = 0; i < histSize; i++) {
			for (int j = 0; j < histSize; j++) {
				// TODO
				int im1Val = (int) (im1.getPixelValue(i, j));
				int im2Val = (int) (im2.getPixelValue(i, j));
				jH.setElementValue(im1Val, im2Val, jH.getElement(im1Val, im2Val) + 1);
			}
		}
		...
		for (int i = 0; i < histSize; i++) {
			for (int j = 0; j < histSize; j++) {
				// TODO
				jH.setElementValue(i, j, jH.getElement(i, j) / (double) (histSize * histSize));
			}
		}
	...
	public static void main(String[] args){
		...
		// Load images
		String imageDataLoc = "D:/Dropbox/Tobias/Eclipse Workspace/CONRAD/src/edu/stanford/rsl/tutorial/dmip/";
		String filename1 = imageDataLoc + "T1.png";
		String filename2 = imageDataLoc + "Proton.png";
	
		exercise82 ex82obj = new exercise82();
		
		Grid2D image1 = ex82obj.readImageFile(filename1);
		Grid2D image2 = ex82obj.readImageFile(filename2);
		...
		ex82obj.reference = image1_blurred;
		ex82obj.image = image2_blurred;
		
		// Perform Optimization
		double res[] = ex82obj.performOptimization();
		
		// Stack for visualization purposes only
		Grid3D optimizationStepsGrid = new Grid3D(ex82obj.reference.getWidth(), ex82obj.reference.getHeight(), ex82obj.iteration, false);
		for (int i = 0; i < optimizationStepsGrid.getSize()[2]; i++) {
			optimizationStepsGrid.setSubGrid(i, ex82obj.movingStack.getSubGrid(i));
		}
