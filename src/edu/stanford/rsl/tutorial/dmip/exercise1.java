...
public class exercise1 {
	...
	public static void invertSVD(SimpleMatrix A)
	{
		...
		int sInd = Slowrank.getDiag().getLen(); //TODO
		...
		System.out.println("Slowrank = " + Slowrank.toString());
		SimpleMatrix templowrank = SimpleOperators.multiplyMatrixProd(svd.getU(), Slowrank);
		SimpleMatrix Alowrank = SimpleOperators.multiplyMatrixProd(A, templowrank);//TODO
		...
		SimpleVector xn = SimpleOperators.subtract(x, xr); //TODO
		...
		//TODO
		System.out.println("percentage: " + xn.normL2() * 100 + "%");
		
		// computation number
		double cn = svd.cond();
		System.out.println("condition number: " + cn);
	}
	...
	public static void optimizationProblem1(SimpleMatrix A, int rankDeficiency)
	{
		...
		//compute A0
		SimpleMatrix A0 = SimpleOperators.multiplyMatrixProd(svd.getU(), SimpleOperators.multiplyMatrixProd(Slowrank, svd.getV().transposed()));
		//TODO
		//TODO
	...
	public static void optimizationProblem3(Grid2D image, int rank)
	{
		...
		for(int k = 0; k < rank; k++)
		{
			...
			//TODO
			error[k] = op.rmse(image, imageRank);
		}
	...
	public static void optimizationProblem4(double[] xCoords, double[] yCoords)
	{
		...
		SimpleVector aCol = new SimpleVector(xCoords);
		SimpleVector y = new SimpleVector(xCoords);
		//TODO
		//TODO
		//TODO
	...
	public static void main(String[] args) 
	{
		...
		String filename = "D:/FAU Erlangen/Semester/2. Semester/Diagnostic Medical Image Processing/Übung 1/mr_head_angio.jpg";
	...
	public static void understandingSVD()
	{
		System.out.println("\nUnderstanding SVD:");
		
		SimpleMatrix cir = new SimpleMatrix(1, 2);
		cir.setElementValue(0, 0, 3 * Math.sqrt(3));
		cir.setElementValue(0, 1, 3);
		System.out.println("circle: " + cir.toString());
		DecompositionSVD svdCir = new DecompositionSVD(cir);
		System.out.println("left singular vectors: " + svdCir.getU().toString());
		System.out.println("singluar values: " + svdCir.getS().toString());
		System.out.println("right singular vectors: " + svdCir.getV().toString() + "\n");
		
		SimpleMatrix tri = new SimpleMatrix(1, 2);
		tri.setElementValue(0, 0, -1);
		tri.setElementValue(0, 1, Math.sqrt(3));
		System.out.println("triangle: " + tri.toString());
		DecompositionSVD svdTri = new DecompositionSVD(tri);
		System.out.println("left singular vectors: " + svdTri.getU().toString());
		System.out.println("singluar values: " + svdTri.getS().toString());
		System.out.println("right singular vectors: " + svdTri.getV().toString() + "\n");
		
		
		SimpleMatrix both = new SimpleMatrix(2, 2);
		both.setElementValue(0, 0, 3 * Math.sqrt(3));
		both.setElementValue(1, 0, 3);
		both.setElementValue(0, 1, -1);
		both.setElementValue(1, 1, Math.sqrt(3));
		System.out.println("both: " + both.toString());
		DecompositionSVD svdBoth = new DecompositionSVD(both);
		System.out.println("left singular vectors: " + svdBoth.getU().toString());
		System.out.println("singluar values: " + svdBoth.getS().toString());
		System.out.println("right singular vectors: " + svdBoth.getV().toString());
	}
}
