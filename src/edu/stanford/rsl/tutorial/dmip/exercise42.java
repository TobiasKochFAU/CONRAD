	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < corrected.getHeight(); i++)
		{
			for (int j = 0; j < corrected.getWidth(); j++)
			{
				...
				else
				{
					...
					for(int m = i - kernelSize/2; m < i + kernelSize/2; m++) 
					{
						for(int n = j - kernelSize/2; n < j + kernelSize/2; n++) 
						{
							if(m >= 0 && m < mu_ij.getHeight() && n >= 0 && n < mu_ij.getWidth()) 
							{
								float currentVal = mu_ij.getPixelValue(n, m);
								if (currentVal > 0)
								{
									localMean += currentVal;
									numNonZeros++;
								}
							}
						}
					}
					localMean /= numNonZeros;
					tempVal = localMean / mu; 
				}
				// TODO: apply the bias field correction for the multiplicative model
				corrected.putPixelValue(j, i, (mu_ij.getPixelValue(j, i) / tempVal));
			}
		}
		...
		for (int k = 0; k < fftimage.getHeight(); k++){
			for (int l = 0; l < fftimage.getWidth(); l++){	
				 ...
				 * TODO
				 */
				double val = 1 - beta * Math.exp(- ((Math.pow(fk, 2) + Math.pow(fl, 2))/(2 * Math.pow(sigma, 2))));
				fftimage.putPixelValue(l, k, val);
			}
		}
		 ...
		 */
		Grid2D corrected3 = ex4Object.highPass(biased, 0.01);
