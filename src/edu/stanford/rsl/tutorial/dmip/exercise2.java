...
public class exercise2 {
	...
	public void doImageUndistortion()
	{
		...
		float fx = (float) imageWidth / nx;
		float fy = (float) imageHeight / ny;
		...
		for(int i = 0; i < nx; i++)
		{
			for(int j = 0; j < ny; j++)
			{
				...
				Xu.setElementValue(j, i, xprime.getAtIndex((int)(i*fx), (int)(j*fy)));
				Yu.setElementValue(j, i, yprime.getAtIndex((int)(i*fx), (int)(j*fy)));
				Xd.setElementValue(j, i, x.getAtIndex((int)(i*fx), (int)(j*fy)));
				Yd.setElementValue(j, i, y.getAtIndex((int)(i*fx), (int)(j*fy)));
			}
		}
		...
		int numCoeff = (int) ((degree + 1) * (degree + 2)) / 2;	//TODO
		
		// number of Correspondences
		int numCorresp = nx * ny;	//TODO
		...
		for(int r = 0; r < numCorresp; r++)
		{
			...
			for(int k = 0; k <= degree; k++)
			{
				...
				{
					// TODO
					A.setElementValue(r, cc, Math.pow(XuVector.getElement(r), k) * Math.pow(YuVector.getElement(r),  l));
					cc++;
				}
			}
		}
		...
		// TODO
		SimpleVector u = SimpleOperators.multiply(Ainv, XdVector);
		SimpleVector v = SimpleOperators.multiply(Ainv, YdVector);
		...
		for(int i = 0; i < imageWidth; i++)
		{
			for(int j = 0; j < imageHeight; j++)
			{
				...
				for(int k = 0; k <= degree; k++)
				{
					for(int l = 0; l <= degree - k; l++)
					{
						sumX += u.getElement(cc) * Math.pow(xprime.getAtIndex(i, j), k) *
													Math.pow(yprime.getAtIndex(i, j), l);
						sumY += v.getElement(cc) * Math.pow(xprime.getAtIndex(i, j), k) *
													Math.pow(yprime.getAtIndex(i, j), l);
						cc++;
					}
				}
				xDist.setAtIndex(i,	j, sumX);
				yDist.setAtIndex(i,	j, sumY);
			}
		}
		...
		for(int i = 0; i < imageWidth; i++)
		{
			...
			{
				// TODO
				float undistortedValue = InterpolationOperators.interpolateLinear(
						distortedImage, 
						xDist.getAtIndex(i, j) + halfWidth - 0.5f,	// remember: coordinate origin was set to image center
						yDist.getAtIndex(i, j) + halfHeight - 0.5f);
...
//		exObj.distortionField.show("Artificial Distortion Field");
	...
	public exercise2(int caseNo){
		...
		originalImage = ImageUtil.wrapImagePlus(IJ.openImage("D:/FAU Erlangen/Semester/2. Semester/Diagnostic Medical Image Processing/Übung 2/" + filename)).getSubGrid(0);
