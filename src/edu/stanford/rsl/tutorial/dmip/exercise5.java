...
public class exercise5 {
	...
	public Grid1D rampFiltering(Grid1D sinogram, RampFilterType filter){
		...
		if(filter == RampFilterType.RAMLAK)
		{
			...
			for(int i = -paddedSize/2; i < paddedSize - (paddedSize/2); i++)
			{
				int idx = i + (paddedSize/2);
				if(i == 0) {
					ramp.setAtIndex(idx, (float) (1.f / (4.f * Math.pow(deltaS, 2))));
				}
				else if(i % 2 == 1) {
					ramp.setAtIndex(idx, (float) -(1.f / (Math.pow(Math.PI, 2) *
							Math.pow(i * deltaS, 2))));
				}
			}
		}
		else if(filter == RampFilterType.SHEPPLOGAN)
		{
			...
			for(int i = -paddedSize/2; i < paddedSize - (paddedSize/2); i++)
			{
				ramp.setAtIndex(i + (paddedSize/2), (float) (-(2 / Math.pow(Math.PI, 2)) *
						(1 / (4 * Math.pow(i, 2) - 1))));
			}
		}
		else
		...
		for(int i = 0; i < paddedSize; i++) {
			sinogramF.multiplyAtIndex(i, ramp.getAtIndex(i));
		}
	...
	public static void main(String[] args)
	{
		...
		exercise5 parallel = new exercise5();
